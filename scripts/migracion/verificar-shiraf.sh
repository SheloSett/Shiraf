...
echo
psql() { docker exec -i shiraf-db psql -U shiraf -d shiraf -tA "$@"; }
...
CONTEOS=$(psql -c "select string_agg(format('select %L as t, count(*) as n from %I', table_name, table_name), ' union all ' order by table_name) from information_schema.tables where table_schema='public' and table_type='BASE TABLE'" | psql -F $'\t')
